...
var (
	...
	//atagRegExp = regexp.MustCompile(`<a rel=nofollow.*?>.*?</a>`)
	atagRegExp = regexp.MustCompile(`<div class="body-content">.*</div>`)
	num        = 1124
	head       = `<!DOCTYPE html>
<html lang="zh">
<head>
...
func main() {
	...
	go Spy("http://www.wenxue360.com/mingju/1124.html")
...
func Spy(url string) {
	...
	if res.StatusCode == 200 {
		...
		for _, v := range atag {
			...
			if strings.Count(content, "")-1 > 500 {
				ioutil.WriteFile("mingju/"+strconv.Itoa(num)+".html", []byte(head+content+foot), os.ModeAppend)
			}
		}

		if num < 6000 {
			num++
			url := "http://www.wenxue360.com/mingju/" + strconv.Itoa(num) + ".html"
			urlChannel <- url
		} else {
